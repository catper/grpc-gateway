...
func TestParseReqParam(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:             "Test 1",
			expected:         map[string]string{"google/api/annotations.proto": "github.com/catper/grpc-gateway/third_party/googleapis/google/api"},
			request:          "allow_delete_body,allow_merge,allow_repeated_fields_in_body,include_package_in_tags,file=./foo.pb,import_prefix=/bar/baz,Mgoogle/api/annotations.proto=github.com/catper/grpc-gateway/third_party/googleapis/google/api",
		...
		},
		{
			name:             "Test 2",
			expected:         map[string]string{"google/api/annotations.proto": "github.com/catper/grpc-gateway/third_party/googleapis/google/api"},
			request:          "allow_delete_body=true,allow_merge=true,allow_repeated_fields_in_body=true,include_package_in_tags=true,merge_file_name=test_name,file=./foo.pb,import_prefix=/bar/baz,Mgoogle/api/annotations.proto=github.com/catper/grpc-gateway/third_party/googleapis/google/api",
			allowDeleteBodyV: true, allowMergeV: true, allowRepeatedFieldsInBodyV: true, includePackageInTagsV: true,
